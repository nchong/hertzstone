...
void run(struct params *p) {
  ...
  per_iter.push_back(Timer("kernel"));
  per_iter_timings.push_back(std::vector<double>(p->num_iter));

  //internal copies of outputs
  double *force = new double[p->nnode*3];
  double *torque = new double[p->nnode*3];
  double **firstdouble = nl->firstdouble;
  int    **firsttouch = nl->firsttouch;

  for (int run=0; run<p->num_iter; run++) {
    //make copies
    std::copy(p->force,  p->force  + p->nnode*3, force);
    std::copy(p->torque, p->torque + p->nnode*3, torque);
    nl->restore();
    ...
    for (int ii=0; ii<nl->inum; ii++) {
      ...
      for (int jj=0; jj<nl->numneigh[i]; jj++) {
        ...
        pair_interaction(
          ...
          &p->x[(i*3)],     &p->x[(j*3)],
          &p->v[(i*3)],     &p->v[(j*3)],
          &p->omega[(i*3)], &p->omega[(j*3)],
           p->radius[i],     p->radius[j],
           p->mass[i],       p->mass[j],
           p->type[i],       p->type[j],
           shear, touch,
          &force[(i*3)],        &force[(j*3)],
    ...
    if (run == 0) {
      ...
      printf("EXPT %.16f\t%.16f\t%.16f\n",
        p->expected_force[TRACE*3], p->expected_force[(TRACE*3)+1], p->expected_force[(TRACE*3)+2]);
    }
#endif

    if (p->check) {
      check_result(p, nl, force, torque, firstdouble, 0.5, false, false);
    }
  }
